...
<body>
	<div class="container">
		<form action="xulythemsv.php" method="POST">
	 		<div class="form-group row">
	  		...
	  		<div class="form-group row">
			 	...
			 	<div class="col-sm-10">
			  		<select name="idlop" required="required">
						<?php  
                          $con= mysqli_connect("localhost","root","","sinhvien");
                          $sql = " SELECT * FROM tbllop where idlop='".$_SESSION['idlop']."'";
                          $kq = mysqli_query($con,$sql);
                          while ($data = mysqli_fetch_array($kq)) {
                              echo '<option value="'.$data['idlop'].'">'.$data['tenlop'].'</option>';
                          }
						?>
					</select>
		   		</div>
